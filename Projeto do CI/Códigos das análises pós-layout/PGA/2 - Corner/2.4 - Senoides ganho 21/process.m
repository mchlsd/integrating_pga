...
load ('sen70mvppg21.mat')

Va_time = [table2array(senoidG2170mvpp(2:end,1:2:6))];
Va_data = [table2array(senoidG2170mvpp(2:end,2:2:6))];

Vout_time = [table2array(senoidG2170mvpp(2:end,7:2:12))];
Vout_data = [table2array(senoidG2170mvpp(2:end,8:2:12))];

Vcs_time = [table2array(senoidG2170mvpp(2:end,13:2:18))];
Vcs_data = [table2array(senoidG2170mvpp(2:end,14:2:18))];
...
%%nom,ss,sf,fs,ff
figure (1)
plot(Va_time(500:end,1)*1e6,Va_data(500:end,1),'-','LineWidth',1.3,'Color','#EDB120');
hold on
plot(Vout_time(500:end,1)*1e6,Vout_data(500:end,1),'-','LineWidth',1.5);
plot(Vcs_time(500:end,3)*1e6,Vcs_data(500:end,3),'-.','LineWidth',2,'Color','#0072BD');
hold off
%legend('V_A tt @ 27{\circ}C','V_O tt @ 27{\circ}C','V_A ss @ -55{\circ}C','V_O ss @ -55{\circ}C','V_A ff @ 125{\circ}C','V_O ff @ 125{\circ}C','V_{Cs}','Location','North','Orientation','horizontal','Box','on','NumColumns',4,'FontSize',8)
legend('V_A tt @ 27{\circ}C','V_O tt @ 27{\circ}C','V_{Cs}=70 mVpp @ 1 kHz','Location','North','Orientation','horizontal','Box','on','FontSize',8)
...
%exportgraphics(gcf,'PGA_senoid_g21.pdf','ContentType','vector')
