...
for j = 1:numel(theta_list)
    ...
    sMax_NL_gain(:,j) = std(max_NL_gain);
...
subplot(3,2,5);
errorbar(theta_list(2:end),mMax_NL_gain(2:end)*1e6,sMax_NL_gain(2:end)*1e6);
...
plot(x,f);
plot(x,y);
title('\it Slope and first order fit','FontWeight','Normal');
ylabel('V')
xlabel('s')
hold
grid
...
subplot(2,1,2)
plot(x,S*1e6);
title('\it Slope minus first order fit','FontWeight','Normal');
ylabel('\muV')
xlabel('s')
grid