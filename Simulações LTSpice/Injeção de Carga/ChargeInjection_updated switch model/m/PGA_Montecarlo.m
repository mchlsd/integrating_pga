...
while ischar(tline{k})
    ...
    if(findstr('!.param v_vcm',tline{k})) %find SH clock
        i_v_vcm = k;
    end
    
    if(findstr('!.param v_vdiff',tline{k})) %find SH clock
        i_v_vdiff = k;
    end
...
for ix=1:length(v_cm)
    for iy=1:length(v_diff)
        cont = cont+1;
 
        waitbar(cont/(length(v_cm)*length(v_diff)),h,sprintf('V_{cm} = %0.1f V | V_{diff} =  %0.1f V',v_cm(ix),v_diff(iy)))
        ...
        tline{i_v_vdiff} = sprintf('TEXT -968 1024 Left 2 !.param v_vdiff=%0.2f',v_diff(iy));
        tline{i_v_vcm} = sprintf('TEXT -968 984 Left 2 !.param v_vcm=%0.2f',v_cm(ix));
        
        res = findstr('!.tran ', tline{i_tran});
        tline{i_tran} = strcat(tline{i_tran}(1:res), sprintf('.tran 0 %.3fm %.3fm %.1fn startup uic', tsim, tsave, ts));
    ...
    sname = sprintf('Vcm = %0.1f.mat',v_cm(ix));
