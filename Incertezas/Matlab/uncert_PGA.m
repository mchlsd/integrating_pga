...
%
%Equações DESCONSIDERAM aspectos reais da operação do circuito, como o fato
%que cada capacitor já deve estar em regime permanente quando usado por
%outras etapas do circuito.
...
eq1 = e0_sim.*(1-exp(-t./RsCs_sim));
eq2 = 1+(t./RaCa_sim);
eq3 = 1-exp(-t./RoCo_sim);
eq = eq1.*eq2.*eq3;

uncertain = sqrt(d_e0.^2..*(1-exp(-t./RsCs)).^2.*(1+t./RaCa).^2.*(1-exp(-t./RoCo)).^2+d_RsCs.^2.*e0.^2.*t.^2.*(exp(-t./RsCs)).^2.*(1+t./RaCa).^2.*(1-exp(-t./RoCo)).^2./RsCs.^4+d_RaCa.^2.*e0.^2.*(1-exp(-t./RsCs)).^2.*t.^2.*(1-exp(-t./RoCo)).^2./RaCa.^4+d_RoCo.^2.*e0.^2.*(1-exp(-t./RsCs)).^2.*(1+t./RaCa).^2.*t.^2.*(exp(-t./RoCo)).^2./RoCo.^4);
